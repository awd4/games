...
typedef struct tagMTRand {
  unsigned long mt[MT_STATE_VECTOR_LENGTH];
  int index;
} MTRand;
...
unsigned long genRandUniform(MTRand *rand, unsigned long n) {
  const unsigned long rand_max = (unsigned long)0xffffffff;
  if (n == 0 || n == rand_max) {
    return rand_max;
  }
  ...
  while ((sample = genRandLong(rand)) > limit)
    ;
